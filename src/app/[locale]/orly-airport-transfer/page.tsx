...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: 'orlyTransferPage.meta' })
  ...
  return {
    ...
    openGraph: {
      title: t('title'),
      description: t('description'),
      url: `https://prestigeshuttlegroup.com/${locale}/orly-airport-transfer`,
    },
    alternates: {
      canonical: `https://prestigeshuttlegroup.com/${locale}/orly-airport-transfer`,
    },
  }
}

export default async function OrlyAirportTransferPage({
  params,
}: {
  params: Promise<{ locale: string }>
}) {
  ...
  const t = await getTranslations({ locale, namespace: 'orlyTransferPage' })

  const features = [
    { icon: MapPin, key: 'doorToDoor' },
    { icon: Zap, key: 'flightTracking' },
    { icon: Clock, key: 'waiting' },
    { icon: Calendar, key: 'availability' },
    { icon: Shield, key: 'fixedPrice' },
    { icon: UserCheck, key: 'licensed' },
  ] as const
