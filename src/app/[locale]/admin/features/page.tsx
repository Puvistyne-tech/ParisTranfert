...
import { getFeatures, createFeature, updateFeature, deleteFeature } from "@/lib/supabaseService";
...
export default function AdminFeaturesPage() {
  ...
  const [deleteConfirm, setDeleteConfirm] = useState<{ isOpen: boolean; featureKey: string | null }>({
    isOpen: false,
    featureKey: null,
  });
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Features Management</h1>
          <p className="text-gray-600 dark:text-gray-400 mt-2">Manage service features</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="grid gap-4">
          {features.map((feature) => (
            <Card key={feature.key} className="dark:bg-gray-800 dark:border-gray-700">
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-4">
                    ...
                    <div>
                      <h3 className="text-lg font-semibold text-gray-900 dark:text-white">{feature.key}</h3>
                      <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">Icon: {feature.icon}</p>
                      {feature.gradient && (
                        <p className="text-xs text-gray-500 dark:text-gray-500 mt-1">Gradient: {feature.gradient}</p>
                      )}
                    </div>
                  </div>
                  <div className="flex items-center space-x-2">
                    ...
                    <Button 
                      ...
                      onClick={() => setDeleteConfirm({ isOpen: true, featureKey: feature.key })}
                    >
      ...
      {features.length === 0 && !loading && (
        <div className="text-center py-12">
          ...
          <p className="text-gray-600 dark:text-gray-400">Get started by creating a new feature</p>
        </div>
      )}
