...
export const reservationSchema = z.object({
  ...
  passengers: z.union([z.number(), z.string()]).transform((val) => {
...
export const createServiceFieldSchema = (serviceFields: Array<{ fieldKey: string; label: string; required: boolean; fieldType: string }>) => {
  ...
  serviceFields.forEach(field => {
    if (field.required) {
      switch (field.fieldType) {
        case 'number':
          serviceFieldSchema[field.fieldKey] = z.union([z.number(), z.string()]).transform((val) => {
            const num = typeof val === 'string' ? parseFloat(val) : val;
            return isNaN(num) ? 0 : num;
          }).pipe(z.number().min(1, `${field.label} is required`));
          break;
        case 'date':
          serviceFieldSchema[field.fieldKey] = z.string().min(1, `${field.label} is required`);
          break;
        case 'time':
          serviceFieldSchema[field.fieldKey] = z.string().min(1, `${field.label} is required`);
          break;
...
export const locationSchema = z.object({
  ...
  (data) => data.pickup.toLowerCase().trim() !== data.destination.toLowerCase().trim(),
  {
