...
import type { Category, CategoryId, CategoryType } from '@/components/models/categories';
...
import type { VehicleType, VehicleTypeId } from '@/components/models/vehicleTypes';
...
export function mapPricingRow(row: Tables<'service_vehicle_pricing'>): ServiceVehiclePricing {
  return {
    id: row.id,
 ...
 */
export async function getServiceFields(serviceId: string): Promise<ServiceField[]> {
  const { data, error } = await supabase
...
export async function getLocationsByService(serviceId: string): Promise<Location[]> {
  const { data, error } = await supabase
...
export async function updateReservation(
  ...
  if (updates.destinationLocation !== undefined) updateData.destination_location = updates.destinationLocation;
  if (updates.passengers !== undefined) updateData.passengers = updates.passengers;
  if (updates.babySeats !== undefined) updateData.baby_seats = updates.babySeats;
  if (updates.boosterSeats !== undefined) updateData.booster_seats = updates.boosterSeats;
  if (updates.meetAndGreet !== undefined) updateData.meet_and_greet = updates.meetAndGreet;
  if (updates.serviceSubData !== undefined) updateData.service_sub_data = updates.serviceSubData;
 ...
 */
export async function getAllPricing(options: {
  serviceId?: string;
  vehicleTypeId?: string;
  limit?: number;
  offset?: number;
...
export async function updatePricing(
  ...
  if (!checkData || checkData.length === 0) {
    ...
    throw new Error(`Pricing entry with id ${id} not found. It may have been deleted.`);
  }
  ...
  if (error) {
    ...
    throw new Error(`Failed to update pricing: ${error.message} (Code: ${error.code})`);
  }

  // If no data returned (possibly due to RLS), fetch it separately
  if (!data || data.length === 0) {
    console.warn('Update succeeded but no data returned, fetching separately:', { id, updateData });
    ...
    if (fetchError || !fetchedData) {
      ...
      console.warn('Could not fetch updated row, update likely succeeded:', { id, fetchError });
      // We'll let the caller refresh the data instead
      throw new Error(`Update completed but could not retrieve updated data. Please refresh.`);
    }
...
export async function updateCategory(
  ...
  if (updates.name !== undefined) updateData.name = updates.name;
  if (updates.categoryType !== undefined) updateData.category_type = updates.categoryType;
  if (updates.description !== undefined) updateData.description = updates.description;
...
export async function updateVehicleType(
  ...
  if (updates.description !== undefined) updateData.description = updates.description;
  if (updates.image !== undefined) updateData.image = updates.image;
  if (updates.minPassengers !== undefined) updateData.min_passengers = updates.minPassengers;
  if (updates.maxPassengers !== undefined) updateData.max_passengers = updates.maxPassengers;
