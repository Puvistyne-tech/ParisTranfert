      ...
      getClientById(createdReservation.clientId)
        .then((client) => {
          if (client) {
            ...
            sendAdminNotificationEmail(createdReservation.id).catch((err) => {
...
export async function getClientReservations(
  ...
  if (error) {
    ...
    throw new Error(`Failed to fetch client reservations: ${error.message}`);
  }
...
export async function updatePricing(
  ...
  if (checkError) {
    ...
    throw new Error(`Failed to verify pricing entry: ${checkError.message}`);
  }
  ...
  if (!data || data.length === 0) {
    ...
    if (fetchError || !fetchedData) {
      ...
      console.warn("Could not fetch updated row, update likely succeeded:", {
        id,
        fetchError,
      });
...
export async function getHomePageImages(
  ...
  if (error) {
    ...
    throw new Error(`Failed to fetch home page images: ${error.message}`);
  }
...
export async function getAllHomePageImages(
  ...
  if (error) {
    ...
    throw new Error(`Failed to fetch all home page images: ${error.message}`);
  }
...
export async function createHomePageImage(imageData: {
  ...
  if (error) {
    ...
    throw new Error(`Failed to create home page image: ${error.message}`);
  }
...
export async function updateHomePageImage(
  ...
  if (error) {
    ...
    throw new Error(`Failed to update home page image: ${error.message}`);
  }
...
export async function deleteHomePageImage(id: string): Promise<void> {
  ...
  if (error) {
    ...
    throw new Error(`Failed to delete home page image: ${error.message}`);
  }
...
function mapDisneylandHotelRow(
  row: Tables<"disneyland_hotels">,
): DisneylandHotel {
  const raw = row.description_i18n;
  let descriptionI18n: DisneylandHotelDescriptionI18n | null = null;
  if (raw && typeof raw === "object" && !Array.isArray(raw)) {
    descriptionI18n = raw as DisneylandHotelDescriptionI18n;
  }
  const currencyRaw = row.price_currency;
  const priceCurrency: DisneylandHotelPriceCurrency =
    currencyRaw === "usd" ? "usd" : "eur";
  ...
  return {
    ...
    hotelWebsiteUrl: row.hotel_website_url ?? null,
...
export async function getDisneylandHotels(): Promise<DisneylandHotel[]> {
  const { data, error } = await supabase
    .from("disneyland_hotels")
    .select("*")
    .order("display_order", { ascending: true });

  if (error) {
    console.error("Error fetching disneyland hotels:", error);
    throw new Error(`Failed to fetch disneyland hotels: ${error.message}`);
  }

  return (data ?? []).map(mapDisneylandHotelRow);
}

export async function createDisneylandHotel(input: {
  name: string;
  ...
  displayOrder?: number;
  isActive?: boolean;
}): Promise<DisneylandHotel> {
  let displayOrder = input.displayOrder;
  if (displayOrder === undefined) {
    const existing = await getDisneylandHotels();
    displayOrder =
      existing.length > 0
        ? Math.max(...existing.map((h) => h.displayOrder)) + 1
        : 0;
  }
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      hotel_website_url: input.hotelWebsiteUrl?.trim() || null,
  ...
  if (error) {
    console.error("Error creating disneyland hotel:", error);
    throw new Error(`Failed to create disneyland hotel: ${error.message}`);
  }

  return mapDisneylandHotelRow(data);
}

export async function updateDisneylandHotel(
  id: string,
  updates: Partial<{
    name: string;
    descriptionI18n: DisneylandHotelDescriptionI18n | null;
  ...
  const patch: TablesUpdate<"disneyland_hotels"> = {};
  if (updates.name !== undefined) {
    patch.name = updates.name;
  }
  if (updates.descriptionI18n !== undefined) {
    patch.description_i18n = updates.descriptionI18n ?? {};
  }
  if (updates.imageUrl !== undefined) {
    patch.image_url = updates.imageUrl;
  }
  if (updates.googleMapsUrl !== undefined) {
    patch.google_maps_url =
      updates.googleMapsUrl === null || updates.googleMapsUrl === ""
        ? null
        : updates.googleMapsUrl.trim();
  }
  if (updates.address !== undefined) {
    patch.address =
      updates.address === null || updates.address === ""
        ? null
        : updates.address.trim();
  }
  if (updates.starRating !== undefined) {
    patch.star_rating = updates.starRating;
  }
  if (updates.tags !== undefined) {
    patch.tags = updates.tags;
  }
  if (updates.hotelWebsiteUrl !== undefined) {
    patch.hotel_website_url =
      updates.hotelWebsiteUrl === null || updates.hotelWebsiteUrl === ""
        ? null
        : updates.hotelWebsiteUrl.trim();
  }
  if (updates.priceRange !== undefined) {
    patch.price_range = updates.priceRange;
  }
  if (updates.priceCurrency !== undefined) {
    patch.price_currency = updates.priceCurrency;
  }
  if (updates.displayOrder !== undefined) {
    patch.display_order = updates.displayOrder;
  }
  if (updates.isActive !== undefined) {
  ...
  if (error) {
    console.error("Error updating disneyland hotel:", error);
    throw new Error(`Failed to update disneyland hotel: ${error.message}`);
  }

  if (
    previous?.image_url &&
    updates.imageUrl !== undefined &&
    updates.imageUrl !== previous.image_url
  ) {
    await deleteWebsiteImage(previous.image_url, "disneyland-hotel");
  }

  return mapDisneylandHotelRow(data);
}

export async function deleteDisneylandHotel(id: string): Promise<void> {
  ...
  if (fetchError) {
    console.error("Error fetching disneyland hotel:", fetchError);
    throw new Error(`Failed to fetch disneyland hotel: ${fetchError.message}`);
  }
  ...
  if (error) {
    console.error("Error deleting disneyland hotel:", error);
    throw new Error(`Failed to delete disneyland hotel: ${error.message}`);
  }

  if (row?.image_url) {
    await deleteWebsiteImage(row.image_url, "disneyland-hotel");
  }
}

export async function updateDisneylandHotelOrder(
  hotels: DisneylandHotel[],
): Promise<void> {
  for (let i = 0; i < hotels.length; i++) {
    ...
    if (error) {
      console.error("Error updating disneyland hotel order:", error);
      throw new Error(
        `Failed to update disneyland hotel order: ${error.message}`,
      );
    }
  }
}
