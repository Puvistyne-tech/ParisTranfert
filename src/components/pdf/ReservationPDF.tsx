...
export const ReservationPDF: React.FC<ReservationPDFProps> = ({ data, translations }) => {
  // Additional services are free
  const additionalServicesTotal = 0;
  
  // Check if this is an airport transfer service (has pricing)
  const isAirportTransfer = data.serviceName.toLowerCase().includes('airport') || 
                            data.serviceName.toLowerCase().includes('transfer');
  
  // Check if we should show pricing (only for airport transfers with valid price)
  const shouldShowPricing = isAirportTransfer && data.totalPrice > 0;
      ...
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.header}>
          ...
          <View style={styles.headerText}>
            ...
            <Text style={styles.companySubtitle}>{translations.companySubtitle}</Text>
          </View>
        ...
        <View style={styles.tripCard}>
          ...
          <View style={styles.tripInfo}>
            <Text style={styles.tripTitle}>{translations.vehicleTypeService}</Text>
            <View style={styles.tripRow}>
        ...
        <View style={styles.contentGrid}>
          ...
          <View style={styles.leftColumn}>
            ...
            <View style={styles.section}>
              <Text style={styles.sectionTitle}>{translations.vehicleType}</Text>
              <View style={styles.row}>
              ...
              {data.vehicleTypeDescription && (
                <View style={styles.row}>
                  ...
                  <Text style={styles.value}>{data.vehicleTypeDescription}</Text>
                </View>
              )}
            </View>
            ...
            {/* Additional Services */}
            {(data.babySeats > 0 || data.boosterSeats > 0 || data.meetAndGreet) && (
              <View style={styles.section}>
                <Text style={styles.sectionTitle}>{translations.additionalServices}</Text>
                {data.babySeats > 0 && (
                  <View style={styles.row}>
                    <Text style={styles.label}>{translations.babySeats}:</Text>
                    <Text style={styles.value}>{data.babySeats} × {translations.free}</Text>
                  </View>
                )}
                {data.boosterSeats > 0 && (
                  <View style={styles.row}>
                    <Text style={styles.label}>{translations.boosters}:</Text>
                    <Text style={styles.value}>{data.boosterSeats} × {translations.free}</Text>
                  </View>
                )}
                {data.meetAndGreet && (
                  <View style={styles.row}>
                    <Text style={styles.label}>{translations.meetAndGreet}:</Text>
        ...
        <View style={styles.importantNotes}>
          <Text style={styles.importantTitle}>{translations.importantInformation}</Text>
          <Text style={styles.importantText}>• {translations.importantNote1}</Text>
          <Text style={styles.importantText}>• {translations.importantNote2}</Text>
          <Text style={styles.importantText}>• {translations.importantNote3}</Text>
          <Text style={styles.importantText}>• {translations.importantNote4}</Text>
          <Text style={styles.importantText}>• {translations.importantNote5}</Text>
          <Text style={styles.importantText}>• {translations.referenceNumber}: {data.reservationId}</Text>
          <Text style={styles.importantText}>• {translations.created}: {new Date(data.createdAt).toLocaleDateString()}</Text>
        </View>

        {/* Footer */}
        <View style={styles.footer}>
          <View style={styles.footerLeft}>
            <Text style={styles.footerTitle}>{translations.contactInformation}</Text>
            <Text style={styles.footerText}>Email: info@paristransfer.com</Text>
            <Text style={styles.footerText}>Phone: +33 1 23 45 67 89</Text>
            <Text style={styles.footerText}>Website: www.paristransfer.com</Text>
          </View>
          <View style={styles.footerRight}>
            <Text style={styles.footerTitle}>{translations.thankYou}</Text>
            <Text style={styles.footerText}>{translations.thankYouMessage}</Text>
            <Text style={styles.footerText}>{translations.generated}: {new Date().toLocaleString()}</Text>
          </View>
        </View>
      </Page>
    </Document>
